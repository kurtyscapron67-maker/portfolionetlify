    ...
    <section id="social-equilibrium" className="relative bg-bedrock px-6 py-24 md:px-12 md:py-32">
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        >
          ...
          <h2 className="font-heading text-4xl font-bold tracking-tight text-iron md:text-6xl">Staff Impact</h2>
          <p className="max-w-2xl font-body text-base leading-relaxed text-tungsten md:text-lg">
            Management and moderation measured, not narrated. Each cell pulses like a redstone torch — proof of work, not promises.
          </p>
        </motion.div>

        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {metrics.length === 0 ? (
            <p className="col-span-full rounded-sm border border-dashed border-border py-10 text-center font-mono text-xs uppercase tracking-widest text-tungsten/70">no metrics published yet</p>
          ) : (
            metrics.map((m, i) => <MetricCell key={m.id || i} metric={m} index={i} />)
          )}
        </div>
      </div>
    </section>
