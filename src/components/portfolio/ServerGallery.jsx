...
import { X, ChevronLeft, ChevronRight, Server as ServerIcon, Loader2 } from "lucide-react";
...
export default function ServerGallery() {
  ...
  const visible = servers && servers.slice(0, 3);
  const hiddenCount = servers ? Math.max(servers.length - 3, 0) : 0;
  ...
  const activeServer = active !== null && servers ? servers[active] : null;
  const activeImages = activeServer ? serverImages(activeServer) : [];

  const next = useCallback(() => {
    setImgIndex((i) => (activeImages.length <= 1 ? 0 : (i + 1) % activeImages.length));
  }, [activeImages.length]);
  const prev = useCallback(() => {
    setImgIndex((i) => (activeImages.length <= 1 ? 0 : (i - 1 + activeImages.length) % activeImages.length));
  }, [activeImages.length]);
  ...
  const currentImage = activeImages[imgIndex] || activeImages[0];
      ...
      <div className="mx-auto max-w-7xl">
        ...
        >
          ...
          <p className="max-w-2xl font-body text-base leading-relaxed text-tungsten md:text-lg">
            Realms I built, configured, and maintained. Screenshots are fragments of the worlds —
        ...
        {/* states */}
        {servers === null && (
          <div className="flex items-center justify-center py-24 text-tungsten">
            <Loader2 className="h-6 w-6 animate-spin text-redstone" />
            <span className="ml-3 font-mono text-sm uppercase tracking-widest">loading chunks…</span>
          </div>
        )}
        ...
        {servers !== null && servers.length === 0 && (
          <div className="flex flex-col items-center justify-center rounded-sm border border-dashed border-border py-24 text-center">
            ...
            <p className="font-mono text-sm uppercase tracking-widest text-tungsten">no servers registered</p>
            <p className="mt-2 font-body text-sm text-tungsten/70">Add servers from the admin panel to populate the registry.</p>
          </div>
        )}
        ...
        {servers !== null && servers.length > 0 && (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {visible.map((s, i) => (
              <ServerCard key={s.id} server={s} index={i} onOpen={() => openLightbox(i)} />
            ))}

            {hiddenCount > 0 && (
              <button
                onClick={() => openLightbox(3)}
                aria-label={`View ${servers.length} more servers`}
                className="focus-ring group relative flex min-h-[280px] flex-col items-center justify-center overflow-hidden rounded-sm border border-border bg-bedrock"
              >
                <Image
                  src={servers[3].image_url}
                  alt=""
                  className="absolute inset-0 h-full w-full object-cover opacity-20 transition-all duration-500 group-hover:opacity-30 group-hover:scale-105"
                  fittingType="fill"
                />
                <div className="absolute inset-0 bg-void/70" />
                <div className="relative z-10 flex flex-col items-center">
                  <span className="font-heading text-5xl font-bold text-redstone redstone-text-glow md:text-6xl">
                    +{servers.length - 3}
                  </span>
                  <span className="mt-2 font-mono text-[11px] uppercase tracking-[0.25em] text-tungsten">
                    more worlds
                  </span>
                </div>
              </button>
      ...
      <AnimatePresence>
        {activeServer && (
          <motion.div
            ...
            className="fixed inset-0 z-[60] flex items-center justify-center bg-void/90 p-4 backdrop-blur-sm md:p-8"
          >
            <button
              onClick={closeLightbox}
              aria-label="Close"
            ...
            {activeImages.length > 1 && (
              <>
                <button
                  ...
                  className="focus-ring absolute left-3 top-1/2 z-10 flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-sm border border-border bg-bedrock/80 text-iron transition-colors hover:border-redstone hover:text-redstone md:left-6"
                >
                ...
                <button
                  ...
                  className="focus-ring absolute right-3 top-1/2 z-10 flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-sm border border-border bg-bedrock/80 text-iron transition-colors hover:border-redstone hover:text-redstone md:right-6"
                >
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-bedrock to-transparent p-6">
                  <div className="flex flex-wrap items-center gap-3">
                    {activeImages.length > 0 && (
                      <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-redstone">
                        {String(imgIndex + 1).padStart(2, "0")} / {String(activeImages.length).padStart(2, "0")}
                      </span>
                    )}
                    {activeServer.version && (
                      <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-tungsten/70">
                        {activeServer.version}
                      </span>
                    )}
                  </div>
                ...
                <div className="md:col-span-2">
                  ...
                  {activeServer.tags && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {activeServer.tags.split(",").map((t) => t.trim()).filter(Boolean).map((t) => (
                        <span key={t} className="rounded-sm border border-border bg-void px-2.5 py-1 font-mono text-[10px] uppercase tracking-widest text-tungsten">
                          {t}
                        </span>
                      ))}
                    </div>
    ...
    >
      ...
      <div className="relative z-10 p-5">
        <div className="flex items-center gap-2">
          {server.version && (
            <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-redstone">{server.version}</span>
          )}
          {extraCount > 0 && (
            <span className="rounded-sm border border-border bg-void/70 px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-widest text-tungsten">
              +{extraCount} photos
            </span>
          )}
        </div>
